...
phonemes! {
    ...
    /// 先行母音の音色に応じて硬口蓋〜口蓋垂の範囲を連続的に変動します。
    ...
    /// 後部歯茎鼻音 `[n̠]`: r の前
    ///
    /// rの前の撥音について、論者によって見解が一致していません。
    /// 直接的な観測 (rtMRI) において、そもそも r の前で
    /// 他の歯茎音クラスタから物理的に分離した閉鎖位置は確認されていません。
    /// 本ライブラリでは伝統的記述へのオプトインとしてこのラベルを提供しますが、
    /// 特定の調音位置を正解として保証するものではありません。
    Nr   = "Nr",
...
enum NEnvironment {
    ...
    /// (j も同様に扱われることが多いが、これは構音実測による直接の裏付けが
