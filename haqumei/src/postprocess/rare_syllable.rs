...
//!
//! [`crate::Phoneme`] と `jpcommon_rule` は `ヴィ` `テュ` `クィ` など 39 種の
//! 稀な音節をすべて表現でき、Open JTalk の読み変換表もそれらを保っている。
//! 区別を失っているのは辞書のエントリだけで、`unidic-csj` は 20 種を一貫して
//! バ行などの一般的な音節に置き換えている。
//!
//! ```text
...
//! ```
//!
//! そこで復元表は、辞書が一貫して置き換えている音節だけに限る。`デュ` は表に
