...
mod tests {
    ...
    fn test_modify_context_reading() {
        ...
        for (text, expected) in [
            ...
            ("部品等", "ブヒントー"),
            ("主蒸気系等", "シュジョーキケートー"),
            // 「の」を挟んだ 1 つ前の語で決まる。道具なら エ、刀剣なら ツカ
            ("槍の柄を握る", "ヤリノエヲニギル"),
            ("傘の柄", "カサノエ"),
            ("柄杓の柄", "ヒシャクノエ"),
            ("刀の柄が折れる", "カタナノツカガオレル"),
            ...
            ("堀の縁", "ホリノフチ"),
            ("カップの縁", "カップノフチ"),
            // 「〜より外に〜ない」は打ち消しと組でしか成り立たない
            ("それより外に方法はない", "ソレヨリホカニホーホーワナイ"),
            ("そうするより外はあるまい", "ソースルヨリホカワアルマイ"),
            (
                "泣き立てるより外に仕方がなかった",
                "ナキタテルヨリホカニシカタガナカッタ",
            ),
    ...
    fn test_modify_context_reading_negative() {
        ...
        for (text, expected) in [
            ...
            ("平等", "ビョードー"),
            ("等しい", "ヒトシイ"),
            // 「柄」は直前が「の」でなければ、道具の一覧に載っていても発火しない
            ("柄が悪い", "ガラガワルイ"),
            ("大柄な人", "オーガラナヒト"),
            ("家柄の良い", "イエガラノヨイ"),
            // 「筋が収縮する」は キン だが「筋の収縮」は複合語ではないので、
            // 連体化の「の」を飛ばさない限り スジ のまま
            ("筋の収縮", "スジノシューシュク"),
            // 「外に何もない」は ホカ だが、読点が挟まると「何」は別の文節の語なので、
            // 読点で探索を打ち切る限り ソト のまま
            ("外には、何もない", "ソトニワ、ナニモナイ"),
        ] {
            assert_eq!(haqumei.g2k(text).unwrap(), expected, "input: {}", text);
